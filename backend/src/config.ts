...
const envSchema = z.object({
  ...
  if (value.STORY_ENGINE_MODE === 'live' && !value.OPENAI_API_KEY) ctx.addIssue({ code: 'custom', message: 'OPENAI_API_KEY is required in live story mode' });
  if (value.AUDIO_ENGINE_MODE === 'live' && !value.ELEVENLABS_API_KEY) ctx.addIssue({ code: 'custom', message: 'ELEVENLABS_API_KEY is required in live audio mode' });
